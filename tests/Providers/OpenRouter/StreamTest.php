...
describe('approval-required tools', function (): void {
    it('stops streaming when approval-required tool is called (Phase 1)', function (): void {
        FixtureResponse::fakeStreamResponses('v1/chat/completions', 'openrouter/stream-with-approval-tool');
    ...
    it('continues streaming after approval when tool is approved (Phase 2)', function (): void {
        FixtureResponse::fakeStreamResponses('v1/chat/completions', 'openrouter/stream-with-approval-phase2');
        ...
        $messages = [
            ...
            new AssistantMessage(
                content: '',
                toolCalls: [
                    new ToolCall(id: 'call_delete_file_stream', name: 'delete_file', arguments: ['path' => '/tmp/test.txt']),
                ],
                additionalContent: [],
                toolApprovalRequests: [
                    new ToolApprovalRequest(approvalId: 'apr_call_delete_file_stream', toolCallId: 'call_delete_file_stream'),
                ],
            ),
            new ToolResultMessage([], [
                new ToolApprovalResponse(approvalId: 'apr_call_delete_file_stream', approved: true),
            ]),
        ];
