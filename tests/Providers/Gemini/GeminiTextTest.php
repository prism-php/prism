...
describe('Text generation for Gemini', function (): void {
    ...
    it('can generate text with a system prompt from messages', function (): void {
        FixtureResponse::fakeResponseSequence('*', 'gemini/generate-text-with-system-prompt-from-messages');

        $response = Prism::text()
            ->using(Provider::Gemini, 'gemini-1.5-flash')
            ->withMessages([
                new SystemMessage('You are a helpful AI assistant named Prism generated by echolabs'),
    ...
    it('can generate text with multiple system prompts', function (): void {
        FixtureResponse::fakeResponseSequence('*', 'gemini/generate-text-with-multiple-system-prompts');
        ...
        expect($response->text)->toBe("I am Prism, a helpful AI assistant created by echo labs.\n")
            ->and($response->usage->promptTokens)->toBe(22)
            ->and($response->usage->completionTokens)->toBe(14)
            ->and($response->meta->id)->toBe('')
            ->and($response->meta->model)->toBe('gemini-1.5-flash')
            ->and($response->finishReason)->toBe(FinishReason::Stop);
    ...
    it('can generate text using multiple tools and multiple steps', function (): void {
        ...
        expect($firstStep->toolCalls)->toHaveCount(2)
            ...
            ->and($response->usage->promptTokens)->toBe(350)
    ...
    it('can send images from path', function (): void {
        ...
        Http::assertSent(function (Request $request): bool {
            ...
            expect($message[0])->toBe([
                ...
                ->and($message[1]['inline_data']['mime_type'])->toBe('image/png')
    ...
    it('can read process pdf documents', function (): void {
        ...
        Http::assertSent(function (Request $request): bool {
            ...
            expect($message[1])->toBe([
                ...
                ->and($message[0]['inline_data'])->toHaveKeys(['mime_type', 'data'])
                ->and($message[0]['inline_data']['mime_type'])->toBe('application/pdf')
                ->and($message[0]['inline_data']['data'])->toBe(
    ...
    it('can read process text documents', function (): void {
        ...
        Http::assertSent(function (Request $request): bool {
            ...
            expect($message[1])->toBe([
                ...
                ->and($message[0]['inline_data'])->toHaveKeys(['mime_type', 'data'])
                ->and($message[0]['inline_data']['mime_type'])->toBe('text/plain')
                ->and($message[0]['inline_data']['data'])->toBe(
...
describe('search grounding', function (): void {
    it('adds the google_search empty tool if searchGrounding is set to true', function (): void {
        ...
        Http::assertSent(function (Request $request): true {
            ...
            expect($data['tools'][0])->toHaveKey('google_search')
                ->and($data['tools'][0]['google_search'])->toBeObject();
    ...
    it('maps search groundings into additional content', function (): void {
        ...
        expect($response->additionalContent)->toHaveKey('searchEntryPoint')
            ->and($response->additionalContent)->toHaveKey('searchQueries')
            ->and($response->additionalContent)->toHaveKey('groundingSupports')
            ->and($response->additionalContent['searchEntryPoint'])->not()->toBe('')
            ->and($response->additionalContent['searchQueries'])->toHaveCount(1)
            ->and($response->additionalContent['groundingSupports'])->toHaveCount(4)
            ->and($response->additionalContent['groundingSupports'][0])->toBeInstanceOf(MessagePartWithSearchGroundings::class)
            ->and($response->additionalContent['groundingSupports'][0]->text)->not()->toBe('')
            ->and($response->additionalContent['groundingSupports'][0]->startIndex)->not()->toBe(0)
            ->and($response->additionalContent['groundingSupports'][0]->endIndex)->not()->toBe(0)
            ->and($response->additionalContent['groundingSupports'][0]->groundings)->toHaveCount(1)
            ->and($response->additionalContent['groundingSupports'][0]->groundings[0])->toBeInstanceOf(SearchGrounding::class)
            ->and($response->additionalContent['groundingSupports'][0]->groundings[0]->title)->not()->toBe('')
            ->and($response->additionalContent['groundingSupports'][0]->groundings[0]->uri)->not()->toBe('')
            ->and($response->additionalContent['groundingSupports'][0]->groundings[0]->confidence)->not()->toBe(0.0);
...
describe('Cache support for Gemini', function (): void {
    it('can use a cache object with a text request', function (): void {
        ...
        expect($response->text)->toBe("That's the Consolidated Version of the Treaty on the Functioning of the European Union (TFEU), adopted on 25 March 1957.  It outlines the principles, competencies, and policies of the European Union.  The TFEU organizes the EU's functioning and details its areas of competence, including exclusive, shared, and supporting competences.  It also covers non-discrimination, citizenship, and various EU policies (e.g., internal market, agriculture, and justice).  Finally, it sets out institutional and financial provisions.\n")
            ->and($response->usage->promptTokens)->toBe(16)
            ->and($response->usage->completionTokens)->toBe(116)
            ->and($response->usage->cacheReadInputTokens)->toBe(88759);
    });
});
