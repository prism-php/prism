...
describe('Structured output for Z', function (): void {
    ...
    it('handles missing usage data in response', function (): void {
        FixtureResponse::fakeResponseSequence('chat/completions', 'z/structured-missing-usage');

        $schema = new ObjectSchema(
            name: 'InterviewResponse',
            description: 'Structured response',
            properties: [
                new StringSchema('message', 'The message', nullable: false),
                new StringSchema('action', 'The action', nullable: false),
            ],
            requiredFields: ['message', 'action']
        );
    ...
    it('handles responses with missing id and model fields', function (): void {
        FixtureResponse::fakeResponseSequence('chat/completions', 'z/structured-with-missing-meta');

        $schema = new ObjectSchema(
            name: 'Output',
            description: 'Output',
            properties: [
                new StringSchema('weather', 'Weather', nullable: false),
                new StringSchema('game_time', 'Game time', nullable: false),
            ],
            requiredFields: ['weather', 'game_time']
        );

        $response = Prism::structured()
            ->using(Provider::Z, 'z-model')
            ->withSchema($schema)
            ->withPrompt('What time is the game?')
            ->asStructured();
